...
class PointCloudMerger:
    ...
    def callback1(self, msg):
        ...
        while self.find_sync_cloud(msg.header.stamp, self.clouds2) is None or self.find_sync_cloud(msg.header.stamp, self.clouds3) is None:
            rospy.sleep(1e-4)
        ...
        cloud2_rotated = self.rotate(cloud2, -np.pi * 2 / 3)
        cloud3_rotated = self.rotate(cloud3, np.pi * 2 / 3)
        cloud_merged = np.concatenate([cloud1, cloud2_rotated, cloud3_rotated], axis=0)
        cloud_merged_rgb = ros_numpy.point_cloud2.merge_rgb_fields(cloud_merged)
